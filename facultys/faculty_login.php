...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows == 1) {
        ...
        else if (password_verify($password, $row['faculty_password'])) {
...
<body>
  <div class="login-card">
    <h2><i class="fas fa-chalkboard-teacher"></i> Faculty Login</h2>
    <?php if(isset($error)) { echo "<p class='error-msg'>$error</p>"; } ?>
    <form method="POST">
        <div class="mb-3">
            <label class="form-label"><i class="fas fa-envelope"></i> Email</label>
            <input type="email" name="faculty_email" class="form-control" placeholder="Enter your email" required>
        </div>
        <div class="mb-3">
            <label class="form-label"><i class="fas fa-lock"></i> Password</label>
            <input type="password" name="faculty_password" class="form-control" placeholder="Enter your password" required>
        </div>
